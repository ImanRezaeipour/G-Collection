   ...
   public class ReportRepository
    {
       ...
       public List<Report> LoadAllReportsByPerson(int prcPostStatus)
       {
           ...
           try
           {
               ...
               string commandText = @"SELECT NW_Report.Report_Name , NW_Report.Report_FileAddress , NW_Report.Report_ID , NW_Report.Report_IsCalculate 
                                    ...
                                    WHERE NW_AccessReports.Role_Title = @roleTitle";
               SqlParameter[] paramArray = new SqlParameter[1];
               SqlParameter prcParam = new SqlParameter("@roleTitle", roleTitle);
               paramArray[0] = prcParam;
               dtResult = connection.executeSelectQuery(commandText, paramArray);
               if (dtResult == null)
               {
                   throw new MyExceptionHandler("خطا در بارگذاری گزارشات.لطفا مجددا سعی نمایید.");
               }
           ...
           catch (Exception ex)
           {
               ...
               logObj.LayerName = Log.LayerNameEnum.Repository.ToString();
               bool result = logger.InsertToLogTable(logObj);
               throw new MyExceptionHandler("خطا در بارگزاری همه گزارشات.لطفا مجددا سعی نمایید.");
           }
       }
       ...
       public bool DeleteAccessReports(decimal reportId)
       {
           try
           {
       ...
       public bool InsertAccessReports(decimal reportId, Dictionary<string, bool> dicNewValue)
       {
           try
           {
               bool resultInsert = false;
               bool result = false;
               result = DeleteAccessReports(reportId);
               if (result == true)
               {
                   foreach (string reportS in dicNewValue.Keys)
                   {
                       ...
                       SqlParameter report = new SqlParameter("@Report_ID", reportId);
           ...
           catch (Exception ex)
           {
               ...
               logObj.Status = false;
               logObj.MethodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
               logObj.Comment = string.Format("خطا در وارد کردن دسترسی های گزارشات {0}، متن پیغام {1}.خطا", Utility.Utility.GetExecptionMessage(ex));
               logObj.LayerName = Log.LayerNameEnum.Repository.ToString();
               bool BoolResult = new logRepository().InsertToLogTable(logObj);
               throw new MyExceptionHandler("خطا در وارد کردن دسترسی های گزارشات");
           }
       }
    }
}
