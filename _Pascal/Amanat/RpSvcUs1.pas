...
var
   isreader,isdept,newisdept,isPerson : boolean;
...
//kaveh is
  if newisdept then
  begin
    if isdept and isreader then
    begin
      if not isPerson then
        lblgrpp.Caption:=GetDepartmentName(aqryReceipt.FieldByName('Rdr_Department').AsInteger)
      else
        lblgrpp.Caption:= GetDepartmentName(aqryReceipt.FieldByName('Empl_Department').AsInteger);
      lblgrpp.Caption := lblgrpp.Caption + ' - ' + getreadername(aqryReceipt.FieldByName('recit1_Rdr').AsInteger);
    end
    else if isdept then
    begin
      if not isPerson then
    ...
    else if isreader then
    begin
      lblgrpp.Caption := getreadername(aqryReceipt.FieldByName('recit1_Rdr').AsInteger);
    end;
  end
  else
  begin
//kaveh ie
    if isdept
    then lblgrpp.Caption:=GetDepartmentName(aqryReceipt.FieldByName('recit1_Department').AsInteger);
    if isreader
