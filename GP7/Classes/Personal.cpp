...
void Personal::save_traffic (TrafficInformation ti)
{
	...
	if (res)
	{
		...
		s.ShowMessageInvertTemporarily(str);	
                n.ShowMessageInvertTemporarily("ÎæÔ ÂãÏíÏ");
...
string Personal::get_appropriate_message (PersonalMessage index, Language l)
{
	...
	case Farsi:
		...
		case PM_InvalidPersonal:
			message = "ÚÏã ÊØÇÈÞ ˜ÇÑÈÑ";
			break;		
			
		case PM_ErrorWhileSave:
			message = "ÇÔ˜Çá ÏÑ ËÈÊ ÊÑÏÏ";
			break;
                
                case PM_ErrorCardFirst:  
 			message = "ÇÈÊÏÇ ˜ÇÑÊ ÑÇ ÈÐÇÑíÏ";
			break;                 
			
		case PM_HasNotFinger:
			message = "ÝÇÞÏ ÇËÑ ÇäÔÊ";
			break;	

		case PM_PlaceFinger:
			message = "ÇäÔÊ ÎæÏ ÑÇ ÈÐÇÑíÏ";
			break;	
			
		case PM_TimeOut:
			message = "ÇÊãÇã ãåáÊ ÒãÇäí";
			break;		
