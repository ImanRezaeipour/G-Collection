...
uses
  ...
  MSGs, Keyboard, RpGhFsh,slctpart;

type
  TfrmGhFishRpt = class(TForm)
...
procedure TfrmGhFishRpt.SetWinF;
var i : Byte;
begin
end;

procedure TfrmGhFishRpt.CreateReportDBS(fdate,edate:string);
var
  tbl : TAdoTable;
  qry : TAdoQuery;
  ready,readyday : boolean;
  statday : integer;
begin
  ...
  begin
    ...
    then begin
      IsResidentPerson(qry.FieldByName('Fish_PCode').AsString,qry.FieldByName('Fish_Date').AsString,0,ready,readyday,statday);
      if not readyday
      then begin
        tbl.Append;
        ...
        tbl.FieldByName('Gh_statday').AsInteger := statday;
        tbl.Post;
      end;
    end;
    qry.Next;
...
procedure TfrmGhFishRpt.FormCreate(Sender: TObject);
var qry : TAdoQuery;
begin
  SetWinF;
  ...
  begin
    ...
    while not EOF do
    begin
      cmbstation.Items.Add(IntToStr(FieldByName('st_Code').AsInteger) + '-' + FieldByname('st_Name').AsString);
      Next;
    end;
    Close;
...
procedure TfrmGhFishRpt.txtFromDateKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmGhFishRpt.cmbJobKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmGhFishRpt.cmbJobKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 46 then
  begin
...
procedure TfrmGhFishRpt.chkSobhKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmGhFishRpt.rdoScreenKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmGhFishRpt.BitBtn1Click(Sender: TObject);
begin
   PartCode := GetPart;
...
procedure TfrmGhFishRpt.chkSignKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmGhFishRpt.cmdPrintClick(Sender: TObject);
var
  frmGhFish : TfrmRpGhFish;
  part_lcode:string;
  fdate,edate:string;
begin
  ...
  begin
    ...
    Application.CreateForm(TfrmRpGhFish, frmGhFish);
    with frmGhFish do
    begin
      Mouse_Wait;
      ...
      if cmbstation.Text <> '' then
        qryGhFish.SQL.Add('AND P_station = ' + Copy(cmbstation.Text, 1, Pos('-', cmbstation.Text)-1));

      if txtreserve8.Text<>''
      then case cmbbashad.itemindex of
                0:qryGhFish.SQL.Add('AND P_reserve8 = '''+txtreserve8.Text+'''');
                1:qryGhFish.SQL.Add('AND P_reserve8 <> '''+txtreserve8.Text+'''');
           end;
      ...
      begin
        SetUserControl;
        if rdoScreen.Checked then qrptGhFish.Preview
        else qrptGhFish.Print;
        USERCONTROL := False;
      end
      else
        InformationMessage('پرسنل ژتون گرفته با عدم حضور وجود ندارد.');
      qryGhFish.Close;
      Free;
    end;
  end;
