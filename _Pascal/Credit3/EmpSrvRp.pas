...
//          aqryEmplSrvc.SQL.Add('AND Recit1_Department = ' + Copy(cmbDepartment.Text, 1, Pos('-', cmbDepartment.Text)-1));
    ...
    begin
      ...
      begin
        ...
        aqryEmplSrvc.SQL.Add('SELECT EN as EMPL_NO , CR  AS FEE, DT as _DATE, TM as _TIME ,rec as _rec,recc as _recc , empl_Name , empl_Family');
        s := ' From ( ';
        s:=s+' SELECT ChargeDtl.DTLEMPLNO AS EN, ChargeGrp.GRPPRICE AS CR, ChargeGrp.GrpDATE AS DT, ChargeGrp.GRPTIME AS TM, Readers.Rdr_Name AS rec,Readers.Rdr_Code AS recc ';
      ...
      // s := s + 'SELECT CHARGE_EMPLNO as EN ,  CHARGE_PRICE as CR,CHARGE_DATE as DT,CHARGE_time as TM FROM CHARGE ';
        s := s + 'UNION ';
        s := s + 'SELECT RECEIPT1.Recit1_EmplNo AS EN, -RECIT1_TOTALPRICE AS CR, RECEIPT1.Recit1_Date AS DT, RECEIPT1.Recit1_Time AS TM, Readers.Rdr_Name AS rec, Readers.Rdr_Code AS recc FROM Readers INNER JOIN RECEIPT1 ON Readers.Rdr_Code = RECEIPT1.Recit1_Rdr ';
