...
begin
  ...
  qry.SQL.Clear;
//kaveh  qry.SQL.Add('SELECT M_BarCode FROM InputCar WHERE M_IsPark = ' + IntToStr(CNST_TRUE));
  qry.SQL.Add('SELECT N_BarCode FROM NoCard WHERE M_IsDeleted = ' + IntToStr(CNST_FALSE));
  qry.Open;

  cmbBarCode.Items.Clear;
  while not qry.EOF do
  begin
    cmbBarCode.Items.Add((qry.FieldByName('N_BarCode').AsString));
    qry.Next;
  end;
  qry.Close;
//  qry.Close;
...
//kaveh    qry.SQL.Add('SELECT M_BarCode FROM InputCar WHERE M_IsPark = ' + IntToStr(CNST_TRUE));
    qry.SQL.Add('SELECT N_BarCode FROM NoCard WHERE M_IsDeleted = ' + IntToStr(CNST_FALSE));
    qry.SQL.Add('AND N_Number = ''' + txtCarNumber.Text + '''');
    qry.Open;
    if qry.RecordCount > 0 then
    begin
      BCode := qry.FieldByName('N_BarCode').AsString;
