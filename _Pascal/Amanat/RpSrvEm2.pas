...
//kaveh        s := 'ò”— «⁄ »«—';
        if aqryEmplSrvc.FieldByName('_Status').AsInteger = 14 then
          s := 'ò”— „«‰œÂ œÊ—Â'
        else
          s := 'ò”— «⁄ »«—';
...
begin
  ...
  begin
    remain := strtoint(currtostr((CalculBalance(aqryEmplSrvc.FieldByName('EMPL_NO').AsString,'',AddDate(gOptions.fromDate,-1),lastCharge, lastChargeCode)))); //kaveh
    if remain >=0 then
