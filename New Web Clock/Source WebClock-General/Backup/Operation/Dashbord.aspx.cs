...
namespace ASPWebclock
{
    public partial class Dashbord : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                ToDate.Text = GTS.Clock.Infrastructure.Utility.Utility.MiladiToShamsiDate(DateTime.Now);
        ...
        private void FillUnderManagmentPersonnel(decimal webPassID,string state)
        {
            ...
            if (Session["UnserMangmentPerson_Dashboard"] == null)
            {
                ...
                ((DevExpress.Web.ASPxGridView.ASPxGridView)ASPxDropDownEditUnderMangmentPerson.FindControl("ASPxGridView5")).DataSource =(DataTable) Session["UnserMangmentPerson_Dashboard"];
        ...
        private void FillPishcard(decimal webPassID)
        {
            ...
            {
                Session["Pishcards_Dashboard"] = new BUser().GetActivePishCardUserAccessAllowed(webPassID);

            }

            ASPxComboBoxPishcard.DataSource = (DataTable)Session["Pishcards_Dashboard"];
                ...
                foreach (string id in DSParams[5].Split(','))
                {
                    if (id != "")
                    {
                       decimal managerId= Convert.ToDecimal(DashbordsGridView.GetRowValues(DashbordsGridView.FindVisibleIndexByKeyValue(id),"ManagerID"));
                        ...
                        confirmItem.StatusID=new BUser().GetUserStatusBywebPassId(managerId);
        ...
        private string DashbordGridConfirm(DashbordCallbackParam DashbordParam)
        {
            ...
            Person prs = (new BUser()).GetPersonByPostID(Convert.ToInt32(Session["PostID"]));
