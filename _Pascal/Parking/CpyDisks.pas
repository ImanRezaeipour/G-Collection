...
begin
{$I-}
  AssignFile(f, T_P + 'Disk' + IntToStr(DiskNo));
  ReWrite(f);
  for i := 0 to n-1 do
    Writeln(f, FList[i]);
  if NextDisk then
...
begin
  ...
  begin
    ...
    Sizes.Add(IntToStr(FileLen(Files.Strings[i])));
  end;
...
var
  ...
  p : Integer;
begin
  if FileExists(S_P + 'Disk' + IntToStr(DiskNo)) then
  begin
    ReadFileSetup := True;
    NextDisk := False;
    AssignFile(f, S_P + 'Disk' + IntToStr(DiskNo));
    ReSet(f);
    Files.Clear;
    n := 0;
    Top := 0;
