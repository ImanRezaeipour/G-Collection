    ...
    public class BWebpassUser
    {
        ...
        public string DeleteUserAccessToStatusToOverTime(decimal webpassID)
        {
            try
            {
                ...
                WebpassUserProxy webpassObj = GetWebPassUserByID(webpassID);
                bool result = (new WebpassUserRepository().DeleteUserAccessToStatusToOverTime(webpassID));
                if (result == true)
                {
                    Log LogObj = new Log();
                    ...
                    LogObj.Comment = "حذف دسترسی های پرسنل  " + webpassObj.Barcode + " با پست " + webpassObj.PersonPost + " توسط کاربر " + HttpContext.Current.User.Identity.Name + " در تاریخ " + Utility.Utility.ToPersianDate(DateTime.Now) + " در ساعت " + (DateTime.Now.TimeOfDay.Hours.ToString() + ":" + DateTime.Now.TimeOfDay.Minutes.ToString() + ":" + DateTime.Now.TimeOfDay.Seconds.ToString()) + newChanges + " ویرایش گردید.";
                    bool BoolResult = new logRepository().InsertToLogTable(LogObj);
                ...
                else
                {
                    ...
                    throw new MyExceptionHandler("خطا در حذف دسترسی کاربر");
        ...
        public string EditUserAccessToStatusToOverTime(decimal id, int status, int topMan, int maxTimeReq, int maxOverTime, bool agreeAddWork, bool agreeOverTime, bool isAcceptOwner, bool isFinalManager, List<int> pishCardItems)
        {
            try
            {
                ...
                bool result = (new WebpassUserRepository().EditUserAccessToStatusToOverTime(id, status, topMan, maxTimeReq, maxOverTime, agreeAddWork, agreeOverTime, isAcceptOwner, isFinalManager, pishCardItems));
                if (result == true)
                {
                    Log LogObj = new Log();
                    ...
                    LogObj.MethodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                    string newChanges = "";
                    WebpassUserProxy webpassObj = GetWebPassUserByID(id);
                    LogObj.Comment = "تغییر دسترسی های پرسنل  " + webpassObj.Barcode + " با پست " + webpassObj.PersonPost + " توسط کاربر " + HttpContext.Current.User.Identity.Name + " در تاریخ " + Utility.Utility.ToPersianDate(DateTime.Now) + " در ساعت " + (DateTime.Now.TimeOfDay.Hours.ToString() + ":" + DateTime.Now.TimeOfDay.Minutes.ToString() + ":" + DateTime.Now.TimeOfDay.Seconds.ToString()) + newChanges + " ویرایش گردید.";
                    bool BoolResult = new logRepository().InsertToLogTable(LogObj);
                ...
                else
                {
                    ...
                    throw new MyExceptionHandler("خطا در ویرایش دسترسی های کاربر");
        ...
        public string AddNewUserToPerson(string barcode, string userName, string Password, string PasswordRepeat)
        {
            try
            {
                ...
                bool ResultQuery = (new WebpassUserRepository()).AddNewUserToPerson(barcode, userName, login.EncryptPassword(Password));
                if (ResultQuery == true)
                {
                    return "اطلاعات با موفقیت ثبت گردید";
                }
                ...
                {
                    throw new MyExceptionHandler("خطا در تخصیص نام کاربری جدید به پرسنل با barcode = " + barcode);
                }
            }
            catch (MyExceptionHandler ex)
            {
        ...
        public string EditUserOfPerson(string id, string userName, string password, string passwordRepeat)
        {
            try
            {
                ...
                if (password == "")
                {
                    ResultQuery = (new WebpassUserRepository()).EditUserOfPerson(id, userName, password);

                }
                else
                {
                    ResultQuery = (new WebpassUserRepository()).EditUserOfPerson(id, userName, login.EncryptPassword(password));
                }
                if (ResultQuery == true)
                {
                    Log LogObj = new Log();
                    ...
                    LogObj.Comment = " مشخصات کاربری " + " برای پرسنل " + GetPersonByUserName(userName).Barcode + " توسط پرسنل " + "Admin" + " در تاریخ " + Utility.Utility.ToPersianDate(DateTime.Now) + " در ساعت " + (DateTime.Now.TimeOfDay.Hours.ToString() + ":" + DateTime.Now.TimeOfDay.Minutes.ToString() + ":" + DateTime.Now.TimeOfDay.Seconds.ToString()) + newChanges + " ویرایش گردید.";
                    bool BoolResult = new logRepository().InsertToLogTable(LogObj);
                    return "اطلاعات با موفقیت ویرایش گردید";
                }
                else
                {
                    throw new MyExceptionHandler("خطا در ویرایش نام کاربری پرسنل با userName = " + userName);
                }
        ...
        public string DeleteUserOfPerson(string id)
        {
            try
            {
                ...
                WebLoginUserProxy user = new BWebLoginUser().GetUserByID(Convert.ToDecimal(id));
                bool ResultQuery = (new WebpassUserRepository()).DeleteUserToPerson(id);
                if (ResultQuery == true)
                {
                    Log LogObj = new Log();
                    LogObj.Action = "";
                    ...
                    LogObj.Comment = " مشخصات کاربری " + " برای پرسنل " + GetPersonByUserName(user.UserName).Barcode + " توسط پرسنل " + new BWebpassUser().GetPersonByUserName(HttpContext.Current.User.Identity.Name).Barcode + " در تاریخ " + Utility.Utility.ToPersianDate(DateTime.Now) + " در ساعت " + (DateTime.Now.TimeOfDay.Hours.ToString() + ":" + DateTime.Now.TimeOfDay.Minutes.ToString() + ":" + DateTime.Now.TimeOfDay.Seconds.ToString()) + newChanges + " حذف گردید.";
                    bool BoolResult = new logRepository().InsertToLogTable(LogObj);
            ...
            catch (Exception ex)
            {
                ...
                LogObj.UserName = HttpContext.Current.User.Identity.Name;
                LogObj.MethodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                LogObj.Comment = string.Format("خطا در حذف نام کاربری  {0} با id=، متن پیغام {1}.خطا", id, Utility.Utility.GetExecptionMessage(ex));
                bool BoolResult = new logRepository().InsertToLogTable(LogObj);
        ...
        public string EditWebPassPerson(bool isKarkardfrm, bool isRequestfrm, bool isOperatorfrm, bool isKeeperfrm, bool isOverTimefrm, bool isSubPart, bool AccessType, int id)
        {
            try
            {
                bool result = new WebpassUserRepository().EditWebPassPerson(isKarkardfrm, isRequestfrm, isOperatorfrm, isKeeperfrm, isOverTimefrm, isSubPart, AccessType, id);
                if (result == true)
                {
                    Log LogObj = new Log();
                    LogObj.Action = "";
                    ...
                    string newChanges = "";
                    WebpassUserProxy webpassObj = GetWebPassUserByID(id);
                    LogObj.Comment = "تغییر پرسنل تحت مدیریت  " + webpassObj.Barcode + " با پست " + webpassObj.PersonPost + " توسط کاربر " + HttpContext.Current.User.Identity.Name + " در تاریخ " + Utility.Utility.ToPersianDate(DateTime.Now) + " در ساعت " + (DateTime.Now.TimeOfDay.Hours.ToString() + ":" + DateTime.Now.TimeOfDay.Minutes.ToString() + ":" + DateTime.Now.TimeOfDay.Seconds.ToString()) + newChanges + " ویرایش گردید.";
                    bool BoolResult = new logRepository().InsertToLogTable(LogObj);
                    return "اطلاعات با موفقیت ثبت گردید";
                }
                else
        ...
        public string DeleteWebPassPerson(int id)
        {
            try
            {
                ...
                WebpassUserProxy webpassObj = GetWebPassUserByID(id);
                bool result = (new WebpassUserRepository().DeleteWebPassPerson(id));
                if (result == true)
                {
                    Log LogObj = new Log();
                    ...
                    LogObj.Comment = "حذف پرسنل تحت مدیریت  " + webpassObj.Barcode + " با پست " + webpassObj.PersonPost + " توسط کاربر " + HttpContext.Current.User.Identity.Name + " در تاریخ " + Utility.Utility.ToPersianDate(DateTime.Now) + " در ساعت " + (DateTime.Now.TimeOfDay.Hours.ToString() + ":" + DateTime.Now.TimeOfDay.Minutes.ToString() + ":" + DateTime.Now.TimeOfDay.Seconds.ToString()) + newChanges + " ویرایش گردید.";
                    bool BoolResult = new logRepository().InsertToLogTable(LogObj);
                    return "اطلاعات با موفقیت حذف گردید";
                }
                else
                {
                    throw new MyExceptionHandler("خطا در حذف پرسنل تحت مدیریت");
                }
            }
            catch (MyExceptionHandler ex)
            {
