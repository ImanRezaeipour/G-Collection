object frmCoCars: TfrmCoCars
  Left = 82
  Top = 42
  HelpContext = 105
  BiDiMode = bdRightToLeft
  BorderStyle = bsDialog
  Caption = #1605#1593#1585#1601#1610' '#1582#1608#1583#1585#1608#1607#1575#1610' '#1587#1575#1586#1605#1575#1606#1610
  ClientHeight = 332
  ClientWidth = 520
  Color = clBtnFace
  Font.Charset = ARABIC_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Arabic Transparent'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = True
  ParentBiDiMode = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 15
  object grpEdit: TGroupBox
    Left = 0
    Top = 191
    Width = 520
    Height = 135
    Align = alTop
    TabOrder = 1
    DesignSize = (
      520
      135)
    object Label10: TLabel
      Left = 284
      Top = 15
      Width = 34
      Height = 15
      Anchors = [akTop, akRight]
      Caption = #1587#1585#1610#1575#1604' :'
      ExplicitLeft = 159
    end
    object Label17: TLabel
      Left = 256
      Top = 44
      Width = 62
      Height = 15
      Anchors = [akTop, akRight]
      Caption = #1585#1606#1711' '#1582#1608#1583#1585#1608' :'
      ExplicitLeft = 131
    end
    object Label1: TLabel
      Left = 404
      Top = 99
      Width = 18
      Height = 15
      Anchors = [akTop, akRight]
      Caption = #1588'&'#1607#1585
      Enabled = False
      FocusControl = txtCarNumber
      Visible = False
    end
    object Label9: TLabel
      Left = 463
      Top = 44
      Width = 51
      Height = 15
      Anchors = [akTop, akRight]
      Caption = #1606#1575#1605' '#1582#1608#1583#1585#1608' :'
      ExplicitLeft = 338
    end
    object Label8: TLabel
      Left = 448
      Top = 15
      Width = 66
      Height = 15
      Anchors = [akTop, akRight]
      Caption = #1588#1605#1575#1585#1607' '#1582#1608#1583#1585#1608' :'
      ExplicitLeft = 323
    end
    object Label2: TLabel
      Left = 216
      Top = 109
      Width = 50
      Height = 15
      Anchors = [akTop, akRight]
      Caption = #1588#1605#1575#1585#1607' &'#1576#1575#1585#1603#1583
      Enabled = False
      FocusControl = cmbBarCode
      Visible = False
    end
    object Label19: TLabel
      Left = 454
      Top = 73
      Width = 60
      Height = 15
      Anchors = [akTop, akRight]
      Caption = #1588#1605#1575#1585#1607' '#1705#1575#1585#1578' :'
      FocusControl = txtCardID
      ExplicitLeft = 329
    end
    object Label18: TLabel
      Left = 262
      Top = 73
      Width = 56
      Height = 15
      Anchors = [akTop, akRight]
      Caption = #1605#1581#1604' '#1582#1583#1605#1578' :'
    end
    object cmdSave: TBitBtn
      Left = 428
      Top = 106
      Width = 86
      Height = 22
      Anchors = [akTop, akRight]
      Caption = '&'#1579#1576#1578
      TabOrder = 7
      OnClick = cmdSaveClick
      Glyph.Data = {
        
        3055577777555557F7F555000555555999555577755555577755}
      NumGlyphs = 2
    end
    object cmdCancel: TBitBtn
      Left = 7
      Top = 106
      Width = 86
      Height = 22
      Cancel = True
      Caption = '&'#1575#1606#1589#1585#1575#1601
      TabOrder = 8
      OnClick = cmdCancelClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object txtSerial: TEdit
      Left = 201
      Top = 12
      Width = 49
      Height = 23
      Anchors = [akTop, akRight]
      MaxLength = 2
      TabOrder = 1
      OnKeyPress = txtSerialKeyPress
    end
    object cmbCarColor: TComboBox
      Left = 132
      Top = 41
      Width = 118
      Height = 23
      Style = csDropDownList
      Anchors = [akTop, akRight]
      ItemHeight = 15
      TabOrder = 3
      OnEnter = cmbCarNameEnter
      OnKeyPress = cmbCarColorKeyPress
    end
    object cmbCarCity: TComboBox
      Left = 284
      Top = 99
      Width = 114
      Height = 23
      Style = csDropDownList
      Anchors = [akTop, akRight]
      Enabled = False
      ItemHeight = 15
      TabOrder = 10
      Visible = False
      OnEnter = cmbCarNameEnter
      OnKeyPress = cmbCarCityKeyPress
    end
    object cmbCarName: TComboBox
      Left = 324
      Top = 41
      Width = 118
      Height = 23
      Style = csDropDownList
      Anchors = [akTop, akRight]
      ItemHeight = 15
      TabOrder = 2
      OnEnter = cmbCarNameEnter
      OnKeyPress = cmbCarNameKeyPress
    end
    object txtCarNumber: TEdit
      Left = 324
      Top = 12
      Width = 118
      Height = 23
      Anchors = [akTop, akRight]
      MaxLength = 6
      TabOrder = 0
      OnKeyPress = txtCarNumberKeyPress
    end
    object cmbBarCode: TComboBox
      Left = 111
      Top = 101
      Width = 114
      Height = 23
      Style = csDropDownList
      Anchors = [akTop, akRight]
      Enabled = False
      ItemHeight = 15
      MaxLength = 8
      TabOrder = 9
      Visible = False
      OnKeyPress = cmbBarCodeKeyPress
    end
    object txtCardID: TEdit
      Left = 324
      Top = 70
      Width = 118
      Height = 23
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      MaxLength = 8
      ParentBiDiMode = False
      TabOrder = 4
    end
    object cmbDepartment: TComboBox
      Left = 132
      Top = 70
      Width = 118
      Height = 23
      Style = csDropDownList
      Anchors = [akTop, akRight]
      ItemHeight = 15
      TabOrder = 5
      OnChange = cmbDepartmentChange
    end
    object cmbDepartmentCode: TComboBox
      Left = 168
      Top = 72
      Width = 38
      Height = 23
      Style = csDropDownList
      Anchors = [akTop, akRight]
      ItemHeight = 15
      ParentShowHint = False
      ShowHint = True
      TabOrder = 11
      Visible = False
    end
    object chkAllDeparts: TCheckBox
      Left = 7
      Top = 73
      Width = 123
      Height = 17
      Anchors = [akTop, akRight]
      Caption = #1576#1585#1575#1610' '#1705#1604#1610#1607' '#1605#1581#1604#1607#1575#1610' '#1582#1583#1605#1578
      TabOrder = 6
    end
  end
  object grpAll: TGroupBox
    Left = 0
    Top = 0
    Width = 520
    Height = 191
    Align = alTop
    TabOrder = 0
    DesignSize = (
      520
      191)
    object grpSearch: TGroupBox
      Left = 7
      Top = 7
      Width = 507
      Height = 36
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 0
      DesignSize = (
        507
        36)
      object Label3: TLabel
        Left = 420
        Top = 12
        Width = 79
        Height = 15
        Anchors = [akTop, akRight]
        Caption = #1580#1587#1578#1580#1608#1610' '#1582#1608#1583#1585#1608' :'
        FocusControl = txtSearch
        ExplicitLeft = 383
      end
      object rdoBarCode: TRadioButton
        Left = 74
        Top = 13
        Width = 69
        Height = 15
        Alignment = taLeftJustify
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        Caption = #1588#1605#1575#1585#1607' '#1705#1575#1585#1578
        ParentBiDiMode = False
        TabOrder = 3
      end
      object rdoSerial: TRadioButton
        Left = 149
        Top = 13
        Width = 69
        Height = 15
        Alignment = taLeftJustify
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        Caption = #1587#1585#1610#1575#1604' '#1662#1604#1575#1705
        ParentBiDiMode = False
        TabOrder = 2
      end
      object rdoNumber: TRadioButton
        Left = 224
        Top = 13
        Width = 76
        Height = 15
        Alignment = taLeftJustify
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        Caption = #1588#1605#1575#1585#1607' '#1582#1608#1583#1585#1608
        Checked = True
        ParentBiDiMode = False
        TabOrder = 1
        TabStop = True
      end
      object txtSearch: TEdit
        Left = 306
        Top = 9
        Width = 108
        Height = 23
        Anchors = [akTop, akRight]
        TabOrder = 0
        OnChange = txtSearchChange
        OnKeyPress = txtSearchKeyPress
      end
    end
    object dbgCoCars: TDBGrid
      Left = 99
      Top = 49
      Width = 415
      Height = 139
      Anchors = [akLeft, akTop, akRight]
      DataSource = dtaCoCars
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      ReadOnly = True
      TabOrder = 1
      TitleFont.Charset = ARABIC_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -13
      TitleFont.Name = 'Arabic Transparent'
      TitleFont.Style = []
      OnDrawColumnCell = dbgCoCarsDrawColumnCell
      OnKeyDown = dbgCoCarsKeyDown
      OnMouseMove = dbgCoCarsMouseMove
      OnTitleClick = dbgCoCarsTitleClick
    end
    object cmdNew: TBitBtn
      Left = 7
      Top = 49
      Width = 86
      Height = 22
      Caption = #1582#1608#1583#1585#1608' &'#1580#1583#1610#1583
      TabOrder = 2
      OnClick = cmdNewClick
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      NumGlyphs = 2
    end
    object cmdEdit: TBitBtn
      Left = 7
      Top = 78
      Width = 86
      Height = 22
      Caption = #1575'&'#1589#1604#1575#1581' '#1582#1608#1583#1585#1608
      TabOrder = 3
      OnClick = cmdEditClick
      Glyph.Data = {
        
        00333377737FFFFF773333303300000003333337337777777333}
      NumGlyphs = 2
    end
    object cmdDelete: TBitBtn
      Left = 7
      Top = 106
      Width = 86
      Height = 22
      Caption = '&'#1581#1584#1601' '#1582#1608#1583#1585#1608
      TabOrder = 4
      OnClick = cmdDeleteClick
      Glyph.Data = {
        
        05555777775555557F5555000555555505555577755555557555}
      NumGlyphs = 2
    end
    object cmdHelp: TBitBtn
      Left = 7
      Top = 134
      Width = 86
      Height = 22
      Caption = #1585#1575'&'#1607#1606#1605#1575
      TabOrder = 5
      Kind = bkHelp
    end
    object cmdClose: TBitBtn
      Left = 7
      Top = 162
      Width = 86
      Height = 22
      Caption = '&'#1582#1585#1608#1580
      TabOrder = 6
      Kind = bkClose
    end
  end
  object dtaCoCars: TDataSource
    DataSet = tblCoCars
    Left = 344
    Top = 80
  end
  object tblCoCars: TADOTable
    CursorType = ctStatic
    OnCalcFields = tblCoCarsCalcFields
    TableName = 'CoCars'
    Left = 288
    Top = 80
    object tblCoCarsCo_CarNumber: TStringField
      Alignment = taRightJustify
      DisplayLabel = #1588#1605#1575#1585#1607' '#1582#1608#1583#1585#1608
      DisplayWidth = 6
      FieldName = 'Co_CarNumber'
      Size = 6
    end
    object tblCoCarsCo_Serial: TStringField
      DisplayLabel = #1587#1585#1576#1575#1604
      DisplayWidth = 2
      FieldName = 'Co_Serial'
      Size = 2
    end
    object tblCoCarsCo_BarCode: TStringField
      DisplayLabel = #1588#1605#1575#1585#1607' '#1603#1575#1585#1578
      DisplayWidth = 8
      FieldName = 'Co_BarCode'
      Size = 8
    end
    object tblCoCarsCo_City: TStringField
      FieldName = 'Co_City'
      Visible = False
      Size = 15
    end
    object tblCoCarsCo_Name: TStringField
      DisplayLabel = #1606#1575#1605' '#1582#1608#1583#1585#1608
      DisplayWidth = 15
      FieldName = 'Co_Name'
      Size = 15
    end
    object tblCoCarsCo_Color: TStringField
      DisplayLabel = #1585#1606#1711' '#1582#1608#1583#1585#1608
      DisplayWidth = 15
      FieldName = 'Co_Color'
      Size = 15
    end
    object tblCoCarsCo_Depart: TIntegerField
      FieldName = 'Co_Depart'
    end
    object tblCoCarsCo_AllDeparts: TBooleanField
      FieldName = 'Co_AllDeparts'
    end
  end
  object DosMove1: TDosMove
    Active = True
    Options = [moEnter]
    Left = 16
    Top = 192
  end
end
