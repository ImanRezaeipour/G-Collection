...
uses
  ...
  MSGs, Keyboard, RpStatisticsUse;//RpPyman, RpPymanG, RpPymanRejim;

type
  TfrmStatisticsUseRpt = class(TForm)
...
procedure TfrmStatisticsUseRpt.SetWinF;
var i : Byte;
begin
  Caption := (Caption);
...
procedure TfrmStatisticsUseRpt.FormCreate(Sender: TObject);
var
  i : Byte;
  qry : TAdoQuery;
begin
...
procedure TfrmStatisticsUseRpt.txtFromDateKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmStatisticsUseRpt.txtFromDateExit(Sender: TObject);
begin
  if txtFromDate.Text <> '' then txtFromDate.Text := BeautifulDate(txtFromDate.Text);
  g_options.RptFDate := txtFromDate.Text; 
end;

procedure TfrmStatisticsUseRpt.txtToDateKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmStatisticsUseRpt.txtToDateExit(Sender: TObject);
begin
  if txtToDate.Text <> '' then txtToDate.Text := BeautifulDate(txtToDate.Text);
  g_options.RptTDate := txtToDate.Text;
end;

procedure TfrmStatisticsUseRpt.cmbSobhKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 46 then
  begin
...
procedure TfrmStatisticsUseRpt.cmbTerminalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 46 then
  begin
...
procedure TfrmStatisticsUseRpt.cmbNaharKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 46 then
  begin
...
procedure TfrmStatisticsUseRpt.cmbreserve7KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 46 then
  begin
...
procedure TfrmStatisticsUseRpt.cmbBuildingKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 46 then
  begin
...
procedure TfrmStatisticsUseRpt.cmbBuildingKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmStatisticsUseRpt.cmbJobsKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 46 then
  begin
...
procedure TfrmStatisticsUseRpt.cmbMokhalafatKeyDown(Sender: TObject;
...
procedure TfrmStatisticsUseRpt.rdoScreenKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmStatisticsUseRpt.chkSignKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmStatisticsUseRpt.cmdPrintClick(Sender: TObject);
var
  frmRptStatisticsUse : TfrmRptStatisticsUse;
...
//  part_lcode:= GetPartCode(Copy(txtParts.Text, 1, Pos('-', txtParts.Text)-2)) +'%';
  ...
  begin
    Application.CreateForm(TfrmRptStatisticsUse, frmRptStatisticsUse);
    with frmRptStatisticsUse do
    begin
      Mouse_Wait;
...
///////////////
      ...
      qryStatisticsPayment.SQL.Add('select SUM(ff_no) FF_Count,Food_Name, sum(Sobhaneh) SobhanehCnt, sum(Nahar) NaharCnt, sum(Sham) ShamCnt, FF_Date ');
      ...
      qryStatisticsPayment.SQL.Add('fishfood.FF_Food, fishfood.FF_No, foodlist.Food_Type, foodlist.Food_Name, ');
      ...
      qryStatisticsPayment.SQL.Add('case fishfood.FF_Type when 1 then 1 else 0 end Sobhaneh, ');
      qryStatisticsPayment.SQL.Add('case fishfood.FF_Type when 2 then 1 else 0 end Nahar, ');
      qryStatisticsPayment.SQL.Add('case fishfood.FF_Type when 3 then 1 else 0 end Sham ');
      ...
      qryStatisticsPayment.SQL.Add('SELECT fishfood.FF_Date, fishfood.FF_Type, fishfood.FF_FishNo, ');
      qryStatisticsPayment.SQL.Add('fishfood.FF_Food, fishfood.FF_No, foodlist.Food_Type,''غذای میهمان (غذای اضافه)'' Food_Name, ');
      qryStatisticsPayment.SQL.Add('fish.Fish_PCode, fish.Fish_RdrCode, readers.R_Name, ');
      qryStatisticsPayment.SQL.Add('readers.R_Station, building.Building_Name, ');
      qryStatisticsPayment.SQL.Add('case fishfood.FF_Type when 1 then 1 else 0 end Sobhaneh, ');
      qryStatisticsPayment.SQL.Add('case fishfood.FF_Type when 2 then 1 else 0 end Nahar, ');
     ...
     qryStatisticsPayment.SQL.Add(' ) l10');
      ...
      if cmbBuilding.ItemIndex < 0 then
      begin
        qryStatisticsPayment.SQL.Add('Group by Food_Name,FF_Date ');
      end
      else
      begin
        qryStatisticsPayment.SQL.Add('Group by Building_Name,Food_Name,FF_Date ');
...
procedure TfrmStatisticsUseRpt.BitBtn1Click(Sender: TObject);
begin
   PartCode := GetPart;
...
procedure TfrmStatisticsUseRpt.chkMokhPriceKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
