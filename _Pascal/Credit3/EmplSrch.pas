...
begin
  ...
  if cmbsex.itemindex <> -1 then
  begin
    if s <> '' then s := s + ' AND ';
    s := s + 'Empl_Sex = ' + inttostr(cmbsex.itemindex) + '';
  end;
  ...
  if txtSearchNumber.Text <> '' then
  begin
    if s <> '' then s := s + ' AND ';
    s := s + 'Empl_Number LIKE ''' + txtSearchNumber.Text + '%''';
  end;

  if txtSearchName.Text <> '' then
  begin
    if s <> '' then s := s + ' AND ';
    s := s + 'Empl_Name LIKE ''' + txtSearchName.Text + '*''';
  end;
