...
/* trvConceptExpression client events */
    ...
    if (selectedExpressionConceptNode != undefined) {
        ...
        if (InnerInDepth != undefined) {
            if (TlbInterAction_ConceptRuleEditor.get_items().getItemById('tlbItemAdd_TlbInterAction_ConceptRuleEditor') != null) {
                if (InnerInDepth.CandAddToFinal) {
...
/* /trvConceptExpression */

/* trvDetails client events */
...
function trvDetails_Concept_onNodeBeforeRename(sender, eventArgs) {
    ...
    //if (!outterValueConvertedToObject.CandEditInFinal) {
...
function AddExpressionToDetailed() {
    ...
    var newTreeViewNode_Id = DetailedConcept_Selected.ID.toString() + "_" + ExpressionConcept_Selected.ID;
    ...
    // )
    ;
    newTreeViewNode.set_editingEnabled(outterValueConvertedToObject.CandEditInFinal);
        ...
        case "Fa":
            {
                for (i = 0; i < detailsJsonbObject.length; i++) {
                    ...
                    //if (JSON.parse(detail.value)[0] == undefined) {
                    //    expressionObject = JSON.parse(detail.value);
                    //} else if (JSON.parse(detail.value)[0].Value != undefined) {
                    //    expressionObject = JSON.parse(detail.value)[0].Value;
                    //} else if (expressionObject != undefined) {
                    //    expressionObject = JSON.parse(JSON.parse(detail.value));
                    //}

                    if (expressionObject != undefined) {
                        if (expressionObject.ScriptBeginFa != undefined) result += " " + expressionObject.ScriptBeginFa;
                        if (detail.nodes != undefined && detail.nodes.length > 0) result += GenerateScript(detail.nodes, "Fa");
                        if (expressionObject.ScriptEndFa != undefined) result += " " + expressionObject.ScriptEndFa;
                    }
                }
                break;
            }
...
function SingleNodeToPrepare(node) {
    ...
    if (ValueObject != undefined && ValueObject.CandEditInFinal) {
...
function AddToDetailedFromCallback(stringJson, parentId) {
    ...
    for (var i = 0; i < jsonObjectCollection.length; i++) {
        ...
        newTreeViewNode.set_editingEnabled(jsonObjectValueObject.CandEditInFinal);
    ...
    if (jsonObjectCollection != undefined) {
        ...
        for (var i = 0; i < jsonObjectCollection.length; i++) {
            ...
            if (ConceptRule != undefined) {
                ...
                newTreeViewNode.set_editingEnabled(SingleNodeToPrepare(ConceptRule).CandEditInFinal);
