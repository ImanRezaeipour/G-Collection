...
typedef int         FS_CB_GETDISKINFO         (FS_VOLUME  * pVolume,   FS_DISK_INFO * pDiskData);
...
U32      FS_BITFIELD_Read           (const void * pBase, unsigned BitOff, unsigned NumBits);
void     FS_BITFIELD_Write          (U8 * pBase, unsigned BitOff, unsigned NumBits, U32 v);
...
extern U16             FS__MaxSectorSize;
extern U8              FS__IsInited;
...
//#error FS_SUPPORT_MULTIPLE_FS: Multiple simulanteous file systems not yet supported
  ...
  #define FS_GET_DISKINFO(  pVolume, pInfo)                           FS_MAP_GetDiskInfo(pVolume, pInfo)
