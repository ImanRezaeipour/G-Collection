...
type
  TfrmInitCard = class(TForm)
    ...
    lblInfo: TLabel;
    chkPass: TCheckBox;
    adoQryEmpl_Services: TWideStringField;
    adoQryEmpl_Week: TIntegerField;
    ...
    function MakeExtraPosPacket(CardInfo  : TEmployee): String;
...
var
  ...
  a : string;
begin
  tmp.Empl_Services := '1110000';
  A := MakeExtraPosPacket(tmp);
  ConfirmMessage(a);
  exit;
...
begin
  ...
  Result.LastCharge := BeautifulDate(inttostr(ord(PK[14])) + '/' + inttostr(ord(PK[15])) + '/' + inttostr(ord(PK[16])));
...
function TfrmInitCard.MakeExtraPosPacket(CardInfo  : TEmployee): String;
VAR
  I: Integer;
  k : byte;
begin
  k := 0;
  for i := 8 downto 1 do
    if (CardInfo.Empl_Services[i]='1') then
      k := Pow2(8-i)+k;
  result := #0 + #0 + #0 + chr(k);
//kaveh 871217  result := result + Str2PBCD(FixLeft(IntToStr(CardInfo.Empl_CoPost),'0',4));
  result := result + Str2PBCD(FixLeft(IntToHex(CardInfo.Empl_CoPost,2),'0',4));
  result := result + Str2PBCD(FixLeft(IntToStr(CardInfo.Empl_weekPrg),'0',4));
end;
      ...
      case Ord(PK[6]) of
        ...
        begin
            ...
            begin
              ...
              RecWrite.Empl_Services := adoQry.FieldByName('Empl_Services').AsString;
              RecWrite.Empl_weekPrg := adoQry.FieldByName('Empl_week').AsInteger;
              RecWrite.Empl_CoPost := adoQry.FieldByName('Empl_CoPost').AsInteger;
              ...
              begin
                ...
                begin
                  dta := MakeExtraPosPacket(recWrite);
                  Z84_MakePocket(PK ,curRdrCode ,$1D ,0 ,dta);
                  if not SendPocket(PK) then
                    OK := False
                  else
...
//  end;
  if ok
  then begin
       st:= 'òœ Å—”‰·Ì'  + ' -- ' + rec.EmplCode + 'òœ ò«—  ' + rec.EmplID;
       st:=tabdil(st);
//       st:=fixLeft(st,' ',40);
