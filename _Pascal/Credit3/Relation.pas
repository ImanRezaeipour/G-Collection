...
type
  ...
  private
    ...
    function SetWeeks(rdrInfo : TReaders; lst : Tdataset ; idx : integer;var SendCount : integer ; flag : char) : Boolean;
    function SetCoPosts(rdrInfo : TReaders; lst : Tdataset ; idx : integer; inServiceCount : integer;var SendCount : integer ; flag : char) : Boolean;
    function SetServices(rdrInfo : TReaders; lst : Tdataset ; idx : integer;var SendCount : integer ; flag : char) : Boolean;
  public
...
//      HelpContext := 67;
    ...
    begin
      cmdRun.Caption := 'اجرا';
      Caption := 'جداول برنامه هفتگی';
//      HelpContext := 67;
      cmbWeeks.ItemIndex := 0;
      grpWeeks.Visible := True;
      grdRdrs.Align := alTop;
      grpWeeks.Align := alTop;
      grpBtns.Align := alTop;
      AutoSize := True;
    end;
    RDR_COPOSTS:
    begin
      cmdRun.Caption := 'اجرا';
      Caption := 'جدول جایگاه افراد';
//      HelpContext := 67;
      cmbCoPosts.ItemIndex := 0;
      grpCoPosts.Visible := True;
      grdRdrs.Align := alTop;
      grpCoPosts.Align := alTop;
      grpBtns.Align := alTop;
      AutoSize := True;
    end;
    RDR_SERVICES:
...
var
  ...
  cnt: Integer;
  flag: char;
  queryRecordCount, ServiceCount : Integer; //kaveh871218
begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                SQL.Clear;
                sql.add('SELECT Employee.Empl_ID AS empl_id, Min(CHARGEGRP.GrpDATE) AS firstChargeDate, Max(CHARGEGRP.GrpDATE) AS lastChargeDate, Sum(CHARGEGRP.GRPPRICE) AS Credit '+
            ...
            case cmbServices.ItemIndex of
              0 :
                ...
                begin
                  ...
                  Mouse_Wait;
                  Application.ProcessMessages;
                  MSG_Wait('در حال ارسال جداول خدمات ' + '  تعداد : ' + IntToStr(adoQryPersons.RecordCount));

                  if SetServices(Readers[i-1], adoqryPersons,i,charged,'A') then
                    begin
                      grdRdrs.Cells[3, i] := 'ارسال لیست با موفقیت انجام شد . تعداد = ' + IntToStr(adoQryPersons.RecordCount);
                      Z84_MakePocket(A_PK, Readers[i-1].r_Code, $06, 0, ''); //kaveh871219
                      SendPocket(A_PK); //kaveh871219 
                    end
                  else
                    grdRdrs.Cells[3, i] := 'ارسال لیست ناموفق بود . تعداد = '  + IntToStr(charged) + ' از ' + IntToStr(adoQryPersons.RecordCount);
                  grdRdrs.Refresh;
              ...
              1 :
                ...
                begin
                 Application.ProcessMessages;
                  Mouse_Wait;
                  MSG_Wait('در حال حذف جداول برنامه هفتگی ... ');
                  ClearGridMessage;
                  if SetServices(Readers[i-1], nil,i,charged , 'D' ) then   //D fro Delete All Records
                    grdRdrs.Cells[3, i] := 'جداول برنامه هفتگی حذف شد .'
                  else
                    grdRdrs.Cells[3, i] := 'ناموفق';
                  grdRdrs.Refresh;
                  Mouse_NoWait;
...
//                    sql.add('SELECT * FROM Prices ORDER BY P_CoPost, P_Service');
                    Open;
                    queryRecordCount := RecordCount;
                  end;
                  Mouse_Wait;
                  Application.ProcessMessages;
                  MSG_Wait('در حال ارسال جدول جایگاه ها ' + '  تعداد : ' + IntToStr(queryRecordCount));
//kaveh is
                  with adoQryPersons do
                  begin
                    close;
                  ...
                  begin
                    ...
                    Connection := frmDBS.adoConnect;
                    SQL.Clear;
                    sql.add('SELECT * FROM Prices WHERE P_CoPost in (select CoPost_Code from CoPosts)  ORDER BY P_CoPost, P_Service');
                    Open;
                  end;
//kaveh ie
                  if SetCoPosts(Readers[i-1], adoqryPersons, i, ServiceCount, charged, 'A') then
                    begin
                      grdRdrs.Cells[3, i] := 'ارسال لیست با موفقیت انجام شد . تعداد = ' + IntToStr(adoQryPersons.RecordCount);
                      Z84_MakePocket(A_PK, Readers[i-1].r_Code, $06, 0, ''); //kaveh871219
                      SendPocket(A_PK); //kaveh871219
                    ...
                    grdRdrs.Cells[3, i] := 'ارسال لیست ناموفق بود . تعداد = '  + IntToStr(charged) + ' از ' + IntToStr(queryRecordCount);
                  grdRdrs.Refresh;
                  Mouse_NoWait;
                  MSG_NoWait;
                end;
              ...
              1 :
                ...
                begin
                 Application.ProcessMessages;
                  Mouse_Wait;
                  MSG_Wait('در حال حذف جدول جایگاهها ... ');
                  ClearGridMessage;
                  if SetCoPosts(Readers[i-1], nil, i, ServiceCount, charged, 'D' ) then   //D fro Delete All Records
                    grdRdrs.Cells[3, i] := 'جدول جایگاهها حذف شد .'
                  else
                    grdRdrs.Cells[3, i] := 'ناموفق';
                  grdRdrs.Refresh;
                  Mouse_NoWait;
            ...
            case cmbWeeks.ItemIndex of
              0 :
                ...
                begin
                  ...
                  Mouse_Wait;
                  Application.ProcessMessages;
                  MSG_Wait('در حال ارسال جداول برنامه هفتگی ' + '  تعداد : ' + IntToStr(adoQryPersons.RecordCount));

                  if SetWeeks(Readers[i-1], adoqryPersons,i,charged,'A') then
                    grdRdrs.Cells[3, i] := 'ارسال لیست با موفقیت انجام شد . تعداد = ' + IntToStr(adoQryPersons.RecordCount)
                  else
                    grdRdrs.Cells[3, i] := 'ارسال لیست ناموفق بود . تعداد = '  + IntToStr(charged) + ' از ' + IntToStr(adoQryPersons.RecordCount);
                  grdRdrs.Refresh;
              ...
              1 :
                ...
                begin
                 Application.ProcessMessages;
                  ...
                  MSG_Wait('در حال حذف جداول برنامه هفتگی ... ');
                  ClearGridMessage;
                  if SetWeeks(Readers[i-1], nil,i,charged , 'D' ) then   //D fro Delete All Records
                    grdRdrs.Cells[3, i] := 'جداول برنامه هفتگی حذف شد .'
                  else
                    grdRdrs.Cells[3, i] := 'ناموفق';
                  grdRdrs.Refresh;
    ...
    begin
      case Ord(G_PK[4]) of
        ...
        begin
          ...
          PacketRecCount := (Ord(G_PK[2])-4) div 15;
          for j :=0 to PacketRecCount-1 do
          begin
            ClkRecs[j] := Z84_AnalizeData(copy(G_PK,6+(j*15),15), ErrRec);
            ...
            if (befRec.rdrCode <> ClkRecs[j].rdrCode) or
               ...
               (befRec.rdrPayment <> ClkRecs[j].rdrPayment) then
            begin
              befRec := ClkRecs[i];
              if not ErrRec then
                Inc(RecNo)
...
function TfrmRelations.SetWeeks(rdrInfo: TReaders; lst: Tdataset; idx: integer;var SendCount: integer; flag: char): Boolean;
var
  k : integer;
  dta : string;
  function send(ST : string;status :char) : boolean;
      ...
      begin
        ...
        begin
          if not ComPortInitial(rdrInfo) then
            ...
            Result := Z84_SetWeeksCoPostsServices(ST,ord(Status));
        end
        else
          Result := TCP_UDP_SetWeeksCoPostsServices(rdrInfo, ST,ord(Status));
      end;
    else
      Result := False;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        dta := 'W';
        dta := dta + fixleft(Str2PBCD(IntToHex(lst.FieldByName('W_Code').AsInteger,2)),#0,2);
        dta := dta + Str2PBCD(DelColon(DelComa(copy(lst.FieldByName('W_PrgDay1').AsString,7,47))));
        dta := dta + Str2PBCD(DelColon(DelComa(copy(lst.FieldByName('W_PrgDay2').AsString,7,47))));
        dta := dta + Str2PBCD(DelColon(DelComa(copy(lst.FieldByName('W_PrgDay3').AsString,7,47))));
        dta := dta + Str2PBCD(DelColon(DelComa(copy(lst.FieldByName('W_PrgDay4').AsString,7,47))));
        dta := dta + Str2PBCD(DelColon(DelComa(copy(lst.FieldByName('W_PrgDay5').AsString,7,47))));
        dta := dta + Str2PBCD(DelColon(DelComa(copy(lst.FieldByName('W_PrgDay6').AsString,7,47))));
        dta := dta + Str2PBCD(DelColon(DelComa(copy(lst.FieldByName('W_PrgDay7').AsString,7,47))));
        inc(k);
        if not send(dta,flag) then
        begin
          Result := False;
...
function TfrmRelations.SetCoPosts(rdrInfo: TReaders; lst: Tdataset;idx: integer; inServiceCount : integer; var SendCount: integer; flag: char): Boolean;
var
  k,li : integer;
  dta : string;
  function send(ST : string;status :char) : boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        dta := 'P';
        dta := dta + fixleft(Str2PBCD(IntToHex(lst.FieldByName('P_CoPost').AsInteger,2)),#0,2);
        For li := 1 to inServiceCount do
        begin
          dta := dta + fixleft(Str2PBCD(lst.FieldByName('P_Price').AsString),#0,4);
          if li < inServiceCount then
            Next;
        end;
{kaveh871218
        dta := dta + fixleft(Str2PBCD(IntToHex(lst.FieldByName('CoPost_Code').AsInteger,2)),#0,2);
        dta := dta + fixleft(Str2PBCD(lst.FieldByName('CoPost_S1').AsString),#0,4);
        dta := dta + fixleft(Str2PBCD(lst.FieldByName('CoPost_S2').AsString),#0,4);
        dta := dta + fixleft(Str2PBCD(lst.FieldByName('CoPost_S3').AsString),#0,4);
        dta := dta + fixleft(Str2PBCD(lst.FieldByName('CoPost_S4').AsString),#0,4);
        dta := dta + fixleft(Str2PBCD(lst.FieldByName('CoPost_S5').AsString),#0,4);
        dta := dta + fixleft(Str2PBCD(lst.FieldByName('CoPost_S6').AsString),#0,4);
        dta := dta + fixleft(Str2PBCD(lst.FieldByName('CoPost_S7').AsString),#0,4);  kaveh871218}
        inc(k);
        if not send(dta,flag) then
        begin
          Result := False;
...
function TfrmRelations.SetServices(rdrInfo: TReaders; lst: Tdataset; idx: integer;var SendCount: integer; flag: char): Boolean;
var
  k : integer;
  dta : string;
  function send(ST : string;status :char) : boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        dta := 'T';
        dta := dta + fixleft(lst.FieldByName('S_Code').AsString,'0',2);
        dta := dta + StrRev(Win_To_IR(WIN_MICROSOFT_SYS,copy(lst.FieldByName('S_Name').AsString,1,20)));
        inc(k);
        if not send(dta,flag) then
        begin
          Result := False;
...
SELECT Employee.Empl_ID AS empl_id, Min(CHARGEGRP.GrpDATE) AS firstChargeDate, Max(CHARGEGRP.GrpDATE) AS lastChargeDate, Sum(CHARGEGRP.GRPPRICE) AS Credit
...
$00 $00 $11 $11 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00
$00 $00 $00 $00 $22 $22 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00
$00 $00 $00 $00 $00 $00 $33 $33 $00 $00 $00 $00 $00 $00 $00 $00
$00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00
$00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00
$00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00 $00
