...
ADOTable2.Open;   
  ...
  grdFields.RowCount := FLD_Max+1;

  grdFields.Cells[1, 1] := ('رديف');
  grdFields.Cells[2, 1] := '50';

  grdFields.Cells[1, 2] := ('کد');
  grdFields.Cells[2, 2] := '80';

  grdFields.Cells[1, 3] := ('نام خانوادگي - نام');
  grdFields.Cells[2, 3] := '150';

  grdFields.Cells[1, 4] := ('مركز');
  grdFields.Cells[2, 4] := '100';

  grdFields.Cells[1, 5] := ('محل خدمت');
  grdFields.Cells[2, 5] := '100';

  grdFields.Cells[1, 6] := ('جايگاه افراد');
  grdFields.Cells[2, 6] := '100';

  grdFields.Cells[1, 7] := ('گروه تخفيف');
  grdFields.Cells[2, 7] := '100';

  grdFields.Cells[1, 8] := ('وضعيت');
  grdFields.Cells[2, 8] := '75';

  grdFields.Cells[1, 9] := ('موجودي');
  grdFields.Cells[2, 9] := '100';

  grdFields.Cells[1, 9] := ('موجودي');
  grdFields.Cells[2, 9] := '100';

  grdFields.Cells[1, 10] := ('شماره پرسنلی');
  grdFields.Cells[2, 10] := '100';

  grdFields.Cells[1, 11] := ('کد ملی');
  grdFields.Cells[2, 11] := '100';

  grdFields.Cells[1, 12] := ('تلفن');
  grdFields.Cells[2, 12] := '100';

  grdFields.Cells[1, 13] := ('آدرس');
  grdFields.Cells[2, 13] := '300';

  grdFields.Cells[1, 14] := ('جنسیت');
  grdFields.Cells[2, 14] := '60';
...
begin
  ...
  begin
    ErrorMessage('هيچ فيلدي انتخاب نشده است.');
    grdFields.SetFocus;
  end
  else if not IsWidthValid then
  begin
    ErrorMessage('عرض فيلدهاي انتخابي نادرست است.');
    grdFields.SetFocus
  end
  else
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ErrorMessage('عرض فيلدهاي انتخاب شده از عرض كاغذ بيشتر است.');

        aqryEmployee.Close;
      end
      else
        ErrorMessage('پرسنلي براي گزارش وجود ندارد.');
