...
*   Rest  P2.8  PINSEL4 00
*   ComE  P3.24 PINSEL7 00
*   MdmE  P3.25 PINSEL7 00  
*********************************************************************/
  PINSEL4_bit.P2_0=2;
  PINSEL4_bit.P2_1=2;
  PINSEL4_bit.P2_2=2;
...
/*****************************************************************************/
void Hardware::Com1Enable(bool EN)
{
  if(EN)
  {
    FIO3CLR_bit.P3_24= 1;
...
{
  U1IER = IER_THRE | IER_RLS | IER_RBR | IER_MDM;             //d.k       
}
/*****************************************************************************/
...
/*****************************************************************************/
void Hardware::ModemInitial(void)
{
  FIO2SET_bit.P2_8 = 1;
  U1MCR = 0x03;
...
/*****************************************************************************/
WORD Hardware::GetPhysicalKey(void)
{
  Utility u;
  WORD Key;//COL-ROW pressed key => 1
  DWORD port_r,port_c;
  Key=0;
  ...
  if((Key & 0xFF00) == 0x0F00)  //col 4
    Key |= 0xE000;

  if((!(Key & 0x00FF)) || (!(Key & 0xFF00)))
    Key=0xFFFF;
...
{
  IO0INTCLR |= KB_Col_MASK;        /* clear interrupt */
}
/*****************************************************************************/
/*LCD*************************************************************************/
/*****************************************************************************/
...
/*SPI*************************************************************************/
/*****************************************************************************/
 void Hardware::InitSPI(void) 
{
  BYTE Rsr;
  PCONP|= (1<<8); 
  PCLKSEL0_bit.PCLK_SPI=0;     //CCLK/4
  Rsr=S0SPSR;
  Rsr=Rsr;

  S0SPCCR  |= 0xC;    //Clock divider
  S0SPCR |= 0x20;     //master mode 8 bit
...
/*****************************************************************************/
void Hardware::SPI( char *buf, int Length )
{
    DWORD i;
    Utility  u;
   // char Dummy;
