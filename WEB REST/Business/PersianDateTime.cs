...
namespace Business
{
    public class PersianDateTime
    {
        ...
        private void CheckYear(int YearNo)
        {
            ...
            {
                throw new Exception(String.Format("مقدار {0} برای سال نامعتبر است", YearNo));
            }
        }
        ...
        private void CheckMonth(int MonthNo)
        {
            ...
            {
                throw new Exception(String.Format("مقدار {0} برای ماه نامعتبر است", MonthNo));
            }
        }
        ...
        private void CheckDay(int YearNo, int MonthNo, int DayNo)
        {
            if (MonthNo < 6 && DayNo > 31)
                throw new Exception(String.Format("مقدار {0} برای روز در 6 ماه اول سال نامعتبر است", DayNo));

            if (MonthNo > 6 && DayNo > 30)
                throw new Exception(String.Format("مقدار {0} برای روز در 6 ماه دوم سال نامعتبر است", DayNo));

            if (MonthNo == 12 && DayNo > 29)
            {
                if (!pc.IsLeapDay(YearNo, MonthNo, DayNo) || DayNo > 30)
                    throw new Exception(String.Format("مقدار {0} برای روز در سال ها کبیسه نامعتبر است", DayNo));
            }
        }
        ...
        private void CheckHour(int HourNo)
        {
            ...
            {
                throw new Exception(String.Format("مقدار {0} برای ساعت نامعتبر است", HourNo));
            }
        }
        ...
        private void CheckMinute(int MinuteNo)
        {
            ...
            {
                throw new Exception(String.Format("مقدار {0} برای دقیقه نامعتبر است", MinuteNo));
            }
        }
        #endregion
        ...
        public static PersianDateTime Parse(string Value, bool IncludesTime)
        {
            ...
            if (IncludesTime)
            {
                if (Value.Length > 19)
                {
                    throw new Exception(String.Format("مقدار تاریخ {0} نامعتبر است", Value));
                }
        ...
        public static PersianDateTime Parse(string Value, string Format)
        {
            switch (Format)
            {
                ...
                default:
                    throw new Exception(String.Format("PesianDatTime.Pars({0}, {1})", Value, Format));
            }
        }
        ...
        public static PersianDateTime Parse(string Value)
        {
            ...
            throw new Exception(String.Format("فرمت مقدار تاریخ {0} نامعتبر است", Value));
        }
        ...
        {
            if (FromDate > ToDate)
                throw new Exception("تاریخ شروع از پایان بزرگتر است");
            if (FromDate == ToDate)
                return 1;
        ...
        public static string GetEndOfShamsiMonthString(string persianYear, string persianMonth)
        {
            PersianCalendar pc = new PersianCalendar();
            int endDay = pc.GetDaysInMonth(Convert.ToInt32(persianYear),Convert.ToInt32(persianMonth));
            return persianYear.ToString() + "/" + persianMonth.ToString() + "/" + endDay.ToString();
