    ...
    public class BRequest
    {
                                                  ...
                                                  , int Clock_FinallyAgree, string webPassId, int timeValue, int CountRequest)
        {
            try
            {
                ...
                return (new RequestRepository().InsertRequestPerson(Clock_Date, Clock_SendDate, Clock_AgreeDate, Clock_Time, Clock_eTime, Clock_RdrCode, Clock_Chg
        ...
        public string DeleteRequestByManagerByID(string id)
        {
            try
            {
                return new RequestRepository().DeleteRequestByManagerByID(id);
            }
            catch (MyExceptionHandler ex)
            {
                throw ex;
        ...
        public string DeleteRequestByID(string id)
        {
            try
            {
                ...
                return (new RequestRepository().DeleteRequestByID(id));
            }
            catch (MyExceptionHandler ex)
            {
                throw ex;
        ...
        public string EditRequestDialyByOperator(string barcode, string pishID, string date, string comment, string barcodeOperator, int change, int Readers, int time, string oldPishID, string oldDate)
        {
            try
            {
                ...
                if (newDateYear == oldDateYear && newDateMonth == oldDateMonth)
                {
                    ...
                    return (new RequestRepository().EditRequestDialyByOperator(barcode, pishID, date, comment, barcodeOperator, change, tableName, oldPishID, oldDate));
                }
                else
                {
                    string tableName = "C" + oldDateYear + oldDateMonth;
                    string res = new RequestRepository().DeleteRequestDialyByOperator(barcode, oldPishID, oldDate, time.ToString(), Readers.ToString(), tableName);
                    tableName = "C" + newDateYear + newDateMonth;
                    return new RequestRepository().InsertRequestDialyByOperator(barcode, pishID, date, comment, barcodeOperator, change, Readers, time, tableName).Replace("ثبت", "ویرایش");
                }
            }
            catch (MyExceptionHandler ex)
            {
        ...
        public string EditRequestHourlyPermitByOperator(string barcode, string pishID, string date, string comment, string Operator, int change, int Readers, int fromHour, int toHour, string oldPishID, int oldFromTime, int oldToTime, string oldDate)
        {
            try
            {
                ...
                if (toHour == 0)
                    throw new MyExceptionHandler("فیلد های ساعت را تکمیل نمایید.");
                string tableName = "t" + date.Substring(0, 4) + date.Substring(5, 2);
                return (new RequestRepository().EditRequestHourlyPermitByOperator(barcode, pishID, date, comment, Operator, change, Readers, fromHour, toHour, tableName, oldPishID, oldFromTime, oldToTime, oldDate));
            }
            catch (MyExceptionHandler ex)
            {
                throw ex;
        ...
        public string EditRequestHourlyTrafficByOperator(string barcode, string pishID, string date, string comment, string Operator, int change, int Readers, int time, string oldPishID, int oldTimes, string oldDate)
        {
            ...
            try
            {
                ...
                return (new RequestRepository().EditRequestHourlyTrafficByOperator(barcode, pishID, date, comment, Operator, change, Readers, time, tableName, oldPishID, oldTimes, oldDate));
            ...
            try
            {
                ...
                if (confirmRequestInfo != null)
                {
                    foreach (ConfirmRequestInfo item in confirmRequestInfo)
                    {
                        ...
                        if (FinalyAgree == 0)
                        {
                            string result = DeleteRequestByManagerByID(ID.ToString());
                        }
                        else
                        {
        ...
        public string DeleteRequestDialyByOperator(string barcode, string pishID, string date, string time, string reader)
        {
            try
            {
                ...
                string tableName = "C" + date.Substring(0, 4) + date.Substring(5, 2);
                return (new RequestRepository().DeleteRequestDialyByOperator(barcode, pishID, date, time, reader, tableName));
            }
            catch (MyExceptionHandler ex)
            {
                throw ex;
        ...
        public string DeleteRequestHourlyTrafficByOperator(string barcode, string pishID, string date, string time, string reader)
        {
            try
            {
                ...
                string tableName = "C" + date.Substring(0, 4) + date.Substring(5, 2);
                return (new RequestRepository().DeleteRequestHourlyTrafficByOperator(barcode, pishID, date, time, reader, tableName));
            }
            catch (MyExceptionHandler ex)
            {
                throw ex;
        ...
        public string DeleteRequestHourlyPermitByOperator(string barcode, string pishID, string date, string fromTime, string toTime)
        {
            try
            {
                ...
                string tableName = "t" + date.Substring(0, 4) + date.Substring(5, 2);
                return (new RequestRepository().DeleteRequestHourlyPermitByOperator(barcode, pishID, date, fromTime, toTime, tableName));
            }
            catch (MyExceptionHandler ex)
            {
                throw ex;
                ...
                if (dtResult.Rows.Count > 0)
                {
                    for (int i = 0; i < dtResult.Rows.Count; i++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (!((Enumerable.Range(51, 5).Contains(RecState) && (RegisterdRequestRecState == 3 || RegisterdRequestRecState==86)) || (Enumerable.Range(51, 5).Contains(RegisterdRequestRecState) && (RecState == 3 || RecState==86))))
                                {
        ...
        private void CheckRejectedValidations(string date)
        {
            ...
            catch (Exception ex)
            {
                ...
                LogObj.LayerName = Log.LayerNameEnum.Business.ToString();
                LogObj.UserName = HttpContext.Current.User.Identity.Name;
                LogObj.Comment = string.Format("خطا در کنترول زمان رد کردن  {0}، متن پیغام {1}.خطا", Utility.Utility.GetExecptionMessage(ex));
                bool BoolResult = new logRepository().InsertToLogTable(LogObj);
                throw new MyExceptionHandler("خطا در کنترول زمان رد کردن ");
            }
        }
